...
<div class="pd-20">
    ...
    <span class="l">
        ...
        <shiro:hasPermission name="role:insert">
           <a href="javascript:;" onclick="role_add()" class="btn btn-primary radius"><i class="icon-plus"></i> 添加角色</a>
        </shiro:hasPermission>
    </span>
...
<script type="application/javascript">
    ...
    */
    function operationFormatter(value,row,index){
        var html = '<shrio:hasPermission name="role:update">';
        html +='<a title="编辑" href="javascript:;" onclick="role_edit('+row.roleId+')" style="text-decoration:none;color: #00B83F"><i class="Hui-iconfont"></i></a>&nbsp;&nbsp;&nbsp;';
        html += '</shrio:hasPermission>';
        html += '<shrio:hasPermission name="role:delete">';
        html += '<a title="删除" href="javascript:;" onclick="role_del('+row.roleId+')" class="ml-5" style="text-decoration:none;color: red"><i class="Hui-iconfont"></i></a>';
        html += '</shrio:hasPermission>';
        return html;
    }
...
<script type="application/javascript">
    ...
    function batch_del() {
        ...
        layer.confirm('确认要删除吗？',function (index) {
            var ids = $('#roleTable').bootstrapTable('getAllSelections');
            var roleIds = new Array();
            for (var i=0;i<ids.length;i++){
                roleIds.push(ids[i].roleId);
            }

            $.post("role/batchDelete.do",{roleIds:roleIds},function (data) {
    ...
    function role_edit(id) {
        layer_show("修改角色","role/edit.do?roleId="+id,800,500);
