...
<div class="pd-20">
    ...
    <span class="l">
        ...
        <shiro:hasPermission name="customer:insert">
           <a href="javascript:;" onclick="customer_add()" class="btn btn-primary radius"><i class="icon-plus"></i> 添加客户</a>
        </shiro:hasPermission>
    </span>
...
<script type="application/javascript">
            ...
            */
            ...
            columns: [ //表格显示数据对应的表头设置，
                ...
                */
                ...
                {field: 'gender',title: '客户描述',formatter:genderFormatter},
    ...
    */
    ...
    function operationFormatter(value,row,index){
        var html = '<shrio:hasPermission name="customer:update">';
        html +='<a title="编辑" href="javascript:;" onclick="customer_edit('+row.customerId+')" style="text-decoration:none;color: #00B83F"><i class="Hui-iconfont"></i></a>&nbsp;&nbsp;&nbsp;';
        html += '</shrio:hasPermission>';
        html += '<shrio:hasPermission name="customer:delete">';
        html += '<a title="删除" href="javascript:;" onclick="customer_del('+row.customerId+')" class="ml-5" style="text-decoration:none;color: red"><i class="Hui-iconfont"></i></a>';
        html += '</shrio:hasPermission>';
        return html;
    }
...
<script type="application/javascript">
    ...
    function batch_del() {
        ...
        layer.confirm('确认要删除吗？',function (index) {
            var ids = $('#customerTable').bootstrapTable('getAllSelections');
            var customerIds = new Array();
            for (var i=0;i<ids.length;i++){
                customerIds.push(ids[i].customerId);
            }

            $.post("customer/batchDelete.do",{customerIds:customerIds},function (data) {
    ...
    function customer_edit(id) {
        layer_show("修改客户信息","customer/edit.do?customerId="+id,800,500);

    }

    function customer_add(){
        layer_show("添加客户信息","customer/edit.do",800,500);
    }

    function customer_del(id) {
        layer.confirm('确认要删除吗？',function (index) {
            $.post("customer/delete.do",{customerId:id},function (data) {
                layer.msg(data.msg,{time:1000,icon:data.code});
                if (data.code == 1){
                    refreshTable();
                }
