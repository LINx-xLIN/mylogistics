...
<div class="pd-20">
    ...
    <table id="baseDataTable" class="table table-border table-bordered table-bg">

    <span class="l">
        <shiro:hasPermission name="basicData:batchdelete">
        ...
        <shiro:hasPermission name="basicData:insert">
            <a href="javascript:;" onclick="baseData_add()" class="btn btn-primary radius"><i class="icon-plus"></i> 添加基础数据</a>
        </shiro:hasPermission>
    </span>
...
<script type="application/javascript">
    ...
    */
    ...
    function operationFormatter(value,row,index){
        var html = '<shrio:hasPermission name="basicData:update">';
        html +='<a title="编辑" href="javascript:;" onclick="baseData_edit('+row.baseId+')" style="text-decoration:none;color: #00B83F"><i class="Hui-iconfont"></i></a>&nbsp;&nbsp;&nbsp;';
        html += '</shrio:hasPermission>';
        html += '<shrio:hasPermission name="basicData:delete">';
        html += '<a title="删除" href="javascript:;" onclick="baseData_del('+row.baseId+')" class="ml-5" style="text-decoration:none;color: red"><i class="Hui-iconfont"></i></a>';
        html += '</shrio:hasPermission>';
        return html;
    }
...
<script type="application/javascript">
    ...
    function batch_del() {
        ...
        layer.confirm('确认要删除吗？',function (index) {
            var ids = $('#baseDataTable').bootstrapTable('getAllSelections');
            var baseDataIds = new Array();
            for (var i=0;i<ids.length;i++){
                baseDataIds.push(ids[i].baseId);
            }

            $.post("baseData/batchDelete.do",{baseDataIds:baseDataIds},function (data) {
    ...
    function baseData_edit(id) {
        layer_show("修改基础数据","baseData/edit.do?baseDataId="+id,800,500);
    ...
    function baseData_del(id) {
        layer.confirm('确认要删除吗？',function (index) {
            $.post("baseData/delete.do",{baseDataId:id},function (data) {
                layer.msg(data.msg,{time:1000,icon:data.code});
                if (data.code == 1){
                    refreshTable();
                }
